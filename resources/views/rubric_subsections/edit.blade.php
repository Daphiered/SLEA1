...
<div class="container my-4">
  <h3 class="mb-3">Edit Rubric Subsection #{{ $sub->sub_items }}</h3>
  ...
  <form method="POST" action="{{ route('rubric-subsections.update',$sub) }}" class="card p-3">
    @csrf
    @method('PUT')
      ...
      <select name="section_id" class="form-select @error('section_id') is-invalid @enderror" required>
        ...
        @foreach($sections as $section_id => $title)
          <option value="{{ $section_id }}"
            {{ old('section_id', $sub->section_id) == $section_id ? 'selected' : '' }}>
            {{ $title }}
          </option>
        @endforeach
      </select>
    ...
    <div class="mb-3">
      ...
      <input name="sub_section" class="form-control @error('sub_section') is-invalid @enderror"
             maxlength="255" value="{{ old('sub_section', $sub->sub_section) }}" required>
    ...
    <div class="mb-3">
      ...
      <input name="evidence_needed" class="form-control @error('evidence_needed') is-invalid @enderror"
             maxlength="255" value="{{ old('evidence_needed', $sub->evidence_needed) }}">
    ...
    <div class="mb-3">
      ...
      <input type="number" name="order_no" min="1" max="255"
             ...
             value="{{ old('order_no', $sub->order_no) }}" required>
