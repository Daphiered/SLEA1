...
<div class="container my-4">
  ...
  <form action="{{ route('rubric-categories.store') }}" method="POST" class="card p-3">
    @csrf
    <div class="mb-3">
      <label class="form-label">Title</label>
      <input name="title" class="form-control" maxlength="50" value="{{ old('title') }}" required>
    </div>
    <div class="mb-3">
      <label class="form-label">Max Points</label>
      <input name="max_points" type="number" step="0.01" min="0" max="999.99" 
             class="form-control" value="{{ old('max_points') }}" required>
    </div>
    <div class="mb-3">
